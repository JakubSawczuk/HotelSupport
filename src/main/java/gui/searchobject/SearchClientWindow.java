...
public class SearchClientWindow implements Runnable, IStandardGUIclass {
    ...
    public void setup(){
        ...
        TypedQuery<Client> queryClient = SupportDatabase.entityManager
                .createQuery("SELECT e FROM Client e " +
                        "WHERE Pesel = '9608002555'", Client.class);
        List<Client> clientList = queryClient.getResultList();

        updateTab(clientList.get(0));
